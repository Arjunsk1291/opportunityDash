    ...
    if (!matchesStatus) {
      return {
        ...
        reasonMeta: {
          ...
          note: 'Filter uses getDisplayStatus() (avenirStatus AWARDED overrides everything; otherwise tenderResult wins; otherwise canonicalStage).',
        },
      };
    }
  }
...
const Dashboard = () => {
  ...
  const kpiCards = [
    {
      label: 'Tenders Regretted',
      value: groupedBuckets.regretted.groups.length,
      chip: `${currency === 'AED' ? '' : '$'}${formatCompactNumber(convertValue(groupedBuckets.regretted.rows.reduce((sum, row) => sum + Number(row.opportunityValue || 0), 0)))}`,
      tone: 'text-slate-700',
    ...
    },
    {
      ...
      chip: `${currency === 'AED' ? '' : '$'}${formatCompactNumber(convertValue(groupedBuckets.hold.rows.reduce((sum, row) => sum + Number(row.opportunityValue || 0), 0)))}`,
      tone: 'text-amber-600',
      glow: 'analytics-kpi-glow-amber',
      icon: PauseCircle,
      type: 'hold' as const,
    },
    {
      ...
      chip: `${currency === 'AED' ? '' : '$'}${formatCompactNumber(convertValue(groupedBuckets.won.value || 0))} total`,
    ...
    },
    {
      ...
      glow: 'analytics-kpi-glow-emerald',
      icon: Target,
      chip: `${groupedBuckets.won.groups.length} won projects`,
      type: 'value' as const,
    },
    {
      label: 'Tenders Lost',
      value: groupedBuckets.lost.groups.length,
      chip: `${currency === 'AED' ? '' : '$'}${formatCompactNumber(convertValue(groupedBuckets.lost.rows.reduce((sum, row) => sum + Number(row.opportunityValue || 0), 0)))}`,
      tone: 'text-rose-600',
    ...
    },
    {
      ...
      chip: `${groupedBuckets.winRatio.wonCount} won out of ${groupedBuckets.winRatio.resolvedCount} decided`,
