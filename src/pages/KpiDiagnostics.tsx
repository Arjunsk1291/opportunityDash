...
const DiagnosticsTable = ({ title, rows }: { title: string; rows: KpiDiagnosticEntry[] }) => (
  <div className="analytics-card p-4 sm:p-5">
    <h2 className="text-sm font-semibold text-foreground">{title} ({rows.length})</h2>
    <div className="mt-3 overflow-x-auto">
      <table className="w-full min-w-[1280px] text-left text-xs">
        ...
        <tbody>
          {rows.map((row) => (
            <tr key={`${title}-${row.id}`} className="border-b border-border/40 align-top text-foreground">
    ...
    return (
      <div className="space-y-4">
        <div className="analytics-card p-6">
          <h1 className="text-lg font-semibold text-foreground">KPI Diagnostics</h1>
          <p className="mt-2 text-sm text-muted-foreground">No diagnostics report found for this window. Re-open from a KPI card click.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      <div className="analytics-card p-5">
        <h1 className="text-lg font-semibold text-foreground">KPI Diagnostics: {report.kpiType.toUpperCase()}</h1>
        <p className="mt-1 text-xs text-muted-foreground">Generated at {new Date(report.generatedAt).toLocaleString()}</p>
        {report.truncated ? (
          <div className="mt-3 rounded-lg border border-amber-200 bg-amber-50 p-3 text-xs text-amber-900">
            Report truncated to fit browser storage limits. Narrow filters if you need the full omitted list.
          </div>
        ) : null}
        <div className="mt-4 grid grid-cols-1 gap-2 text-xs text-foreground sm:grid-cols-2 lg:grid-cols-4">
          <div className="rounded-lg border border-border/60 bg-[var(--glass-field-bg)] p-3">Source rows: <strong>{report.counts.sourceRows}</strong></div>
          <div className="rounded-lg border border-border/60 bg-[var(--glass-field-bg)] p-3">Pre-KPI scope: <strong>{report.counts.preKpiScopedRows}</strong></div>
          <div className="rounded-lg border border-border/60 bg-[var(--glass-field-bg)] p-3">Included: <strong>{report.counts.includedRows}</strong></div>
          <div className="rounded-lg border border-border/60 bg-[var(--glass-field-bg)] p-3">Duplicates removed: <strong>{duplicateRows.length}</strong></div>
        </div>
